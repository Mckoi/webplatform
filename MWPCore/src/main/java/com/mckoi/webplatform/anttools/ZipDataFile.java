...
package com.mckoi.webplatform.anttools;
 ...
 * <code>java.util.ZipFile</code>, it uses com.mckoi.data.DataFile under the
...
public class ZipDataFile {
     ...
     */
    private final DataFile archive;
     ...
     */
    public ZipDataFile(DataFile f) throws IOException {
        this(f, null);
    }
     ...
     * @throws IOException if an error occurs while reading the file.
     */
    public ZipDataFile(DataFile f, String encoding) throws IOException {
     ...
     * @throws IOException if an error occurs while reading the file.
     */
    public ZipDataFile(DataFile f, String encoding, boolean useUnicodeExtraFields)
        throws IOException {
      this(f, encoding, useUnicodeExtraFields, true);
    }
     ...
     * @param useUnicodeExtraFields whether to use InfoZIP Unicode
     * Extra Fields (if present) to set the file names.
     * @param parseExtraFields whether to parse the extra fields of
     * the zip entries at all (false means 'useUnicodeExtraFields'
     * is ignored).
     *
     * @throws IOException if an error occurs while reading the file.
     */
    public ZipDataFile(DataFile f, String encoding,
                       boolean useUnicodeExtraFields, boolean parseExtraFields)
        throws IOException {
        this.encoding = encoding;
        this.zipEncoding = ZipEncodingHelper.getZipEncoding(encoding);
        this.useUnicodeExtraFields =
                              parseExtraFields ? useUnicodeExtraFields : false;
        archive = f;

        // This has a side-effect of populating 'entries' and 'nameMap'
...
//        archive.close();
     ...
     */
    public static void closeQuietly(ZipDataFile zipfile) {
        if (zipfile != null) {
            try {
                zipfile.close();
            } catch (IOException e) {
    ...
    private Map populateFromCentralDirectory()
        ...
        positionAtCentralDirectory(archive);

        byte[] cfh = new byte[CFH_LEN];

        byte[] signatureBytes = new byte[WORD];
        archive.get(signatureBytes, 0, signatureBytes.length);
        long sig = ZipLong.getValue(signatureBytes);
        final long cfhSig = ZipLong.getValue(ZipOutputStream.CFH_SIG);
        if (sig != cfhSig && startsWithLocalFileHeader(archive)) {
            throw new IOException("central directory is empty, can't expand"
                                  + " corrupt archive.");
        }
        while (sig == cfhSig) {
            archive.get(cfh, 0, cfh.length);
            int off = 0;
            ...
            archive.get(fileName, 0, fileNameLen);
            ...
            archive.get(cdExtraData, 0, extraLen);
            ze.setCentralDirectoryExtra(cdExtraData);

            byte[] comment = new byte[commentLen];
            archive.get(comment, 0, commentLen);
     ...
     * record.
     */
    private static void positionAtCentralDirectory(DataFile archive)
        throws IOException {
        boolean found = false;
        long off = archive.size() - MIN_EOCD_SIZE;
        long stopSearching = Math.max(0L, archive.size() - MAX_EOCD_SIZE);
        if (off >= 0) {
            archive.position(off);
            InputStream ain = DataFileUtils.asInputStream(archive);
        ...
        archive.position(off + CFD_LOCATOR_OFFSET);
        byte[] cfdOffset = new byte[WORD];
        archive.get(cfdOffset, 0, WORD);
        archive.position(ZipLong.getValue(cfdOffset));
    }
    ...
    private void resolveLocalFileHeaderData(Map entriesWithoutUTF8Flag)
        ...
        while (e.hasMoreElements()) {
            ...
            archive.get(b, 0, SHORT);
            int fileNameLen = ZipShort.getValue(b);
            archive.get(b, 0, SHORT);
            int extraFieldLen = ZipShort.getValue(b);
            archive.position(archive.position() + fileNameLen);
            byte[] localExtraData = new byte[extraFieldLen];
            archive.get(localExtraData, 0, extraFieldLen);
     ...
     */
    private static boolean startsWithLocalFileHeader(DataFile archive)
        throws IOException {
        archive.position(0);
        final byte[] start = new byte[WORD];
        archive.get(start, 0, WORD); //readFully(start);
